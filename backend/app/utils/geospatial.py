...
def buffer_point(
    ...
    for i in range(num_points + 1):  # +1 to close the polygon
        ...
        new_lon_rad = lon_rad + math.atan2(math.sin(angle) * math.sin(distance_rad) * math.cos(lat_rad),
...
def parse_bbox_string(bbox_str: str) -> List[float]:
    ...
    try:
        ...
        raise ValueError(f"Invalid bbox format: {bbox_str}. Expected format: min_lon,min_lat,max_lon,max_lat")
...
def dict_to_geojson(geojson_dict: Dict[str, Any]) -> Union[GeoJSONFeature, GeoJSONFeatureCollection]:
