...
async def analyze_spatial_data(
    ...
    if analysis_type == "centroid":
        result = await db.scalar(
            func.ST_Centroid(spatial_data.geometry)
        )
    elif analysis_type == "area":
        result = await db.scalar(
            func.ST_Area(spatial_data.geometry)
        )
    elif analysis_type == "buffer":
        result = await db.scalar(
            func.ST_Buffer(spatial_data.geometry, 1000)  # 1000 meters buffer
        )
    else:
...
async def find_intersecting_data(
    ...
    intersecting_data = await db.query(SpatialData).filter(
        func.ST_Intersects(SpatialData.geometry, spatial_data.geometry)
    ).all()
