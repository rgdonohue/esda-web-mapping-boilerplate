...
class EnhancedSpatialQueryParams(BaseModel):
    ...
    distance: Optional[float] = Field(
        None, description="Distance in meters for proximity queries", gt=0
    )
    coordinates: Optional[str] = Field(
        None, description="Point coordinates in format: lon,lat or lon,lat,z"
    )
...
def validate_topological_relationship(
    geojson1: Dict[str, Any], geojson2: Dict[str, Any], relationship: str
) -> bool:
