...
async def project_data(
    ...
    await db.execute(
        project_query, {"spatial_data_id": spatial_data_id, "target_srid": target_srid}
    )
    await db.commit()
...
async def simplify_geometry(
    ...
    await db.execute(simplify_query, {"spatial_data_id": spatial_data_id, "tolerance": tolerance})
    await db.commit()
    ...
    return {
        ...
        "reduction": {
            "points": (original_stats.num_points - simplified_stats.num_points)
            / original_stats.num_points,
            "area": (
                abs(float(original_stats.area) - float(simplified_stats.area))
                / float(original_stats.area)
                if original_stats.area and simplified_stats.area
                else None
            ),
        },
    }
...
async def create_buffer(
    ...
    await db.execute(buffer_query, {"spatial_data_id": spatial_data_id, "distance": distance})
    await db.commit()

    return {"message": "Buffer created successfully", "distance": distance}
...
async def generalize_geometry(
    ...
    await db.execute(generalize_query, {"spatial_data_id": spatial_data_id, "tolerance": tolerance})
    await db.commit()
    ...
    return {
        ...
        "reduction": {
            "points": (original_stats.num_points - generalized_stats.num_points)
            / original_stats.num_points,
            "area": (
                abs(float(original_stats.area) - float(generalized_stats.area))
                / float(original_stats.area)
                if original_stats.area and generalized_stats.area
                else None
            ),
        },
    }
