...
class PointGeometry(BaseModel):
    ...
    def validate_coordinates(cls, v):
        if len(v) < 2 or len(v) > 3:
            raise ValueError(
                "Point coordinates must have 2 or 3 elements [lon, lat, (optional)elevation]"
            )
        lon, lat = v[0], v[1]
    ...
    def validate_coordinates(cls, v):
        ...
        for point in v:
            if len(point) < 2 or len(point) > 3:
                raise ValueError(
                    "Each position must have 2 or 3 elements [lon, lat, (optional)elevation]"
                )
        return v
...
class PolygonGeometry(BaseModel):
    ...
    def validate_coordinates(cls, v):
        for ring in v:
            if len(ring) < 4:
                raise ValueError(
                    "Each polygon ring must have at least 4 positions (first and last are the same)"
                )
            if ring[0] != ring[-1]:
...
class SpatialQueryParams(BaseModel):
    bbox: Optional[str] = Field(
        None, description="Bounding box in format: minLon,minLat,maxLon,maxLat"
    )
    distance: Optional[float] = Field(None, description="Distance in meters for proximity queries")
    coordinates: Optional[str] = Field(None, description="Point coordinates in format: lon,lat")
