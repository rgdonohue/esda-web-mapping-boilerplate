...
async def create_choropleth(
    ...
    for i in range(n_classes):
        style_rules.append(
            {"min": breaks[i], "max": breaks[i + 1], "color": colors[i], "fillOpacity": 0.7}
        )

    # Update style in metadata
    metadata["style"] = {"type": "choropleth", "field": field, "rules": style_rules}
    spatial_data.metadata = metadata

    await db.commit()
    return {"message": "Choropleth style created successfully", "style": metadata["style"]}
...
async def create_heatmap(
    ...
    for data in spatial_data:
        ...
        points.append({"coordinates": point["coordinates"], "weight": weight})
    ...
    heatmap_config = {
        "type": "heatmap",
        "points": points,
        "options": {"radius": radius, "blur": blur},
    }
