...
async def calculate_spatial_autocorrelation(
    ...
    values = spatial_data.metadata[field]
    if not isinstance(values, (list, tuple)) or not all(isinstance(x, (int, float)) for x in values):
    ...
    if method == "moran":
        ...
        expected = -1 / (n - 1)
        variance = (n * ((n ** 2 - 3 * n + 3) * np.sum(weights ** 2) - 
                        n * np.sum(weights.sum(axis=1) ** 2) + 
                        3 * np.sum(weights) ** 2) - 
                   (k * ((n ** 2 - n) * np.sum(weights ** 2) - 
